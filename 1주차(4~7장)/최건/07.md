...
선언되지 않은 식별자에 typeof 연산자를 사용할 경우, 참조 오류(Referenece Error)가 발생하지 않고 `undefined`을(를) 반환한다.
...
instanceof 연산자는, 피연산자의 클래스 또는 피연산자의 클래스의 확장인지 확인하는 표현식이다.
